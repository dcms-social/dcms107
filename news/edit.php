...
if (result(query("SELECT COUNT(*) FROM `news` WHERE `id` = '".intval($_GET['id'])."' LIMIT 1",$db), 0) == 0){header("Location: index.php?".SID);exit;}
...
if (isset($_POST['title']) && isset($_POST['msg']) && isset($_POST['link']) && isset($_POST['ok']))
{
	...
	if (!isset($err))
	{
		...
		if ($main_time <= time())
		$main_time = 0;
		query("UPDATE `news` SET `title` = '$title', `msg` = '$msg', `link` = '$link', `main_time` = '$main_time', `time` = '$time' WHERE `id` = '$news[id]' LIMIT 1");
		query("UPDATE `user` SET `news_read` = '0'");
