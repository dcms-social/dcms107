...
if (isset($_POST['title']) && isset($_POST['msg']) && isset($_POST['link']) && isset($_POST['ok']))
{
	...
	query("INSERT INTO `news` (`id_user`,`time`, `msg`, `title`, `main_time`, `link`) values('".$user['id']."','$time', '".$msg."', '$title', '$main_time', '$link')");
	...
	if (isset($_POST['mail'])) // Расслылка новостей на майл
	{
		...
		{
			query("INSERT INTO `mail_to_send` (`mail`, `them`, `msg`) values('$ank[ank_mail]', 'Новости', '".trim(br(bbcode(links(stripcslashes(htmlspecialchars($msg))))))."')");
		}
	}
