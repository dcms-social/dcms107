...
$k_post = result(query("SELECT COUNT(*) FROM `gallery`"),0);
...
$q = query("SELECT * FROM `gallery` ORDER BY `time` DESC LIMIT $start, $set[p_str]");
...
while ($post = fetch_assoc($q))
{
	...
	echo '<a href="/foto/' . $ank['id'] . '/' . $post['id'] . '/">' . text($post['name']) . '</a> (' . result(query("SELECT COUNT(*) FROM `gallery_foto` WHERE `id_gallery` = '$post[id]'"),0) . ' фото)<br />';
