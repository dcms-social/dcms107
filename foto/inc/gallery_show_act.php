...
{
	$q = query("SELECT * FROM `gallery_foto` WHERE `id_gallery` = '$gallery[id]'");
	...
	while ($post = fetch_assoc($q))
	{
		...
		query("DELETE FROM `gallery_komm` WHERE `id_foto` = '$post[id]' LIMIT 1");
		query("DELETE FROM `gallery_foto` WHERE `id` = '$post[id]' LIMIT 1");
		query("DELETE FROM `mark_foto` WHERE `id_foto` = '$post[id]' LIMIT 1");
	}

	if ($user['id'] != $ank['id'])
	admin_log('Фотогалерея','Фотоальбомы',"Удаление альбома " . text($gallery['name']) . " (фотографий: ".rows($q).")");
	query("DELETE FROM `gallery` WHERE `id` = '$gallery[id]' LIMIT 1");
...
{
	query("UPDATE `gallery_foto` SET `avatar` = '0' WHERE `id_user` = '$user[id]'");
	query("INSERT INTO `gallery_foto` (`id_gallery`, `name`, `ras`, `type`, `opis`, `id_user`,`avatar`, `metka`, `time`) values ('$gallery[id]', '$name', 'jpg', 'image/jpeg', '$msg', '$user[id]','1', '$metka', '$time')");
}
else
{
	query("INSERT INTO `gallery_foto` (`id_gallery`, `name`, `ras`, `type`, `opis`, `id_user`, `metka`, `time`) values ('$gallery[id]', '$name', 'jpg', 'image/jpeg', '$msg', '$user[id]', '$metka', '$time')");
}


$id_foto = mysql_insert_id();

query("UPDATE `gallery` SET `time` = '$time' WHERE `id` = '$gallery[id]' LIMIT 1");

$q = query("SELECT * FROM `frends` WHERE `user` = '$user[id]' AND `lenta_foto` = '1' AND `i` = '1'");
...
query("UPDATE `tape` SET `count` = '0' WHERE  `type` = 'album' AND `read` = '1' AND `id_file` = '$gallery[id]'");
...
while ($f = f_array($q))
{
	...
	$lentaSet = f_array(query("SELECT * FROM `tape_set` WHERE `id_user` = '".$a['id']."' LIMIT 1"));
	...
	if  ($f['lenta_foto'] == 1 && $lentaSet['lenta_foto'] == 1)
	{
		...
		if (isset($_GET['avatar']))
		{
			...
			query("INSERT INTO `tape` (`id_user`, `avtor`, `type`, `time`, `id_file`, `count`, `avatar`) values('$a[id]', '$gallery[id_user]', 'avatar', '$time', '$foto[id]', '1', '$avatar[id]')");
		}
		else
		{
			/* Если нет то просто шлем в ленту как новое фото */

			if (result(query("SELECT COUNT(*) FROM `tape` WHERE `id_user` = '$a[id]' AND `type` = 'album' AND `id_file` = '$gallery[id]' LIMIT 1"),0)==0)
			{
				query("INSERT INTO `tape` (`id_user`, `avtor`, `type`, `time`, `id_file`, `count`) values('$a[id]', '$gallery[id_user]', 'album', '$time', '$gallery[id]', '1')");
			}
			else
			{
				$tape = f_array(query("SELECT * FROM `tape` WHERE `type` = 'album' AND `id_file` = '$gallery[id]'"));
				query("UPDATE `tape` SET `count` = '".($tape['count']+1)."', `read` = '0', `time` = '$time' WHERE `id_user` = '$a[id]' AND `type` = 'album' AND `id_file` = '$gallery[id]' LIMIT 1");
			}
		}
	}
...
if ($user['id']==$ank['id']) {
  if (isset($_GET['edit']) && $_GET['edit'] == 'rename' && isset($_GET['ok']) && (isset($_POST['name']) || isset($_POST['opis']))) {
    ...
    if (!isset($err)) {
      if ($user['id'] != $ank['id']) admin_log('Фотогалерея', 'Фотографии', "Переименование альбома пользователя '[url=/id$ank[id]]" . user::nick($ank['id'], 0) . "[/url]'");
      query("UPDATE `gallery` SET `name` = '$name', `privat` = '$privat', `privat_komm` = '$privat_komm', `pass` = '$pass', `opis` = '$msg' WHERE `id` = '$gallery[id]' LIMIT 1");
