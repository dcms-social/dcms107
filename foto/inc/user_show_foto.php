...
$foto2=mysql_fetch_assoc(mysql_query("SELECT `id_gallery`,`id`,`id_user`FROM `gallery_foto` WHERE `id_gallery` = ".(int)$_GET['id_gallery']." AND `id` = ". (int) $_GET['id_foto'] ." AND `id_user` = ". (int) $_GET['id_user']." LIMIT 1"));
 $ank['id'] = $foto2['id_user'];
$gallery['id']=$foto2['id_gallery'];
$foto['id']=$foto2['id'];
$ank = get_user($ank['id']);
...
if (mysql_result(mysql_query("SELECT COUNT(*) FROM `gallery_foto` WHERE `id` = '$foto[id]' AND `id_user` = '$ank[id]' LIMIT 1"),0)==0)
{
  header("Location: /foto/$ank[id]/$gallery[id]/?".SID);
  exit;
}

$foto = mysql_fetch_assoc(mysql_query("SELECT * FROM `gallery_foto` WHERE `id` = '$foto[id]'  AND `id_user` = '$ank[id]' LIMIT 1"));
