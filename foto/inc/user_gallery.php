...
$k_post = result(query("SELECT COUNT(*) FROM `gallery` WHERE `id_user` = '$ank[id]'"),0);
...
$q = query("SELECT * FROM `gallery` WHERE `id_user` = '$ank[id]' ORDER BY `time` DESC LIMIT $start, $set[p_str]");
...
while ($post = fetch_assoc($q))
{
	...
	// Cчетчик фотографий
	$count = result(query("SELECT COUNT(*) FROM `gallery_foto` WHERE `id_gallery` = '$post[id]'"),0);
