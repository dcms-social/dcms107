...
if ($type  ==  'album')
{
	$gallery = fetch_assoc(query("SELECT * FROM `gallery` WHERE `id` = '".$post['id_file']."' LIMIT 1"));
	...
	if ($gallery['id'])
	{
		...
		echo '<div class="nav2">';
		$as = query("SELECT * FROM `gallery_foto` WHERE `id_gallery` = '$gallery[id]' ORDER BY `id` DESC LIMIT $kol");
		
		while ($xx = fetch_assoc($as))
		{
