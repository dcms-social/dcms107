...
if ($type == 'frends')
{
	...
	if ($frend['id'])
	{
		...
		if (result(query("SELECT COUNT(*) FROM `gallery_foto` WHERE `id_user` = '$frend[id]'"),0)>0)
		{
			echo 'Последние добавленные фото ' . user::nick($frend['id'], 0) . '<br />';
			
			$g = query("SELECT * FROM `gallery_foto` WHERE `id_user` = '$frend[id]' ORDER BY `id` DESC LIMIT 4");
			
			while ($xx = fetch_assoc($g))
			{
				$gallery = fetch_assoc(query("SELECT * FROM `gallery` WHERE `id` = '" . $xx['id_gallery'] . "' LIMIT 1"));
