...
{
	$status = fetch_assoc(query("SELECT * FROM `status` WHERE `id` = '" . $post['id_file'] . "' LIMIT 1"));
	...
	if ($status['id'])
	{
		...
		{
			if ($user['id']!=$avtor['id'] && result(query("SELECT COUNT(*) FROM `status_like` WHERE `id_status` = '$status[id]' AND `id_user` = '$user[id]' LIMIT 1"),0)==0){
