...
if ($type == 'obmen')
{
	$dir = fetch_assoc(query("SELECT * FROM `user_files` WHERE `id` = '" . $post['id_file'] . "' LIMIT 1"));
	...
	echo '<div class="nav2">';
	$files = query("SELECT * FROM `obmennik_files` WHERE `my_dir` = '$dir[id]' ORDER BY `id` DESC LIMIT $kol");
