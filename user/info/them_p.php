...
/* Бан недоюзера */
if(result(query("SELECT COUNT(`id`)FROM `user` WHERE `id`='".intval($_GET['id'])."' LIMIT 1"),0)==0){
...
//Если коммы смотрим
if(isset($_GET['komm'])){
$k_post = result(query("SELECT COUNT(`id`) FROM `forum_p` WHERE `id_user`='".$ank['id']."'"), 0);
...
//Если темы смотрим
$k_post = result(query("SELECT COUNT(`id`) FROM `forum_t` WHERE `id_user`='".$ank['id']."'"), 0);
$k_page = k_page($k_post, $set['p_str']);
$page = page($k_page);
$start = $set['p_str'] * $page - $set['p_str'];
$q=query("SELECT id, name, id_forum, id_razdel FROM `forum_t` WHERE `id_user`='".$ank['id']."' ORDER BY `time` DESC LIMIT $start,$set[p_str]");
while($them=fetch_assoc($q)){
echo "<div class='nav1'><a href='/forum/".$them['id_forum']."/".$them['id_razdel']."/".$them['id']."/'>";
echo htmlspecialchars($them['name'])." </a> (".result(query("SELECT COUNT(*)FROM `forum_p` WHERE `id_them`='".$them['id']."'"),0).")";
echo "</div>";
}
