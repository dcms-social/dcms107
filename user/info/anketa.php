...
$timediff=result(query("SELECT `time` FROM `user` WHERE `id` = '$ank[id]' LIMIT 1",$db), 0);
...
echo "<div class='nav1'>";
if (result(query("SELECT COUNT(*) FROM `ban` WHERE `id_user` = '$ank[id]' AND `time` > '$time'"), 0)!=0)
{
$q=query("SELECT * FROM `ban` WHERE `id_user` = '$ank[id]' AND `time` > '$time' ORDER BY `time` DESC LIMIT 5");
...
{
$narush=result(query("SELECT COUNT(*) FROM `ban` WHERE `id_user` = '$ank[id]'"), 0);
...
{
$ank_coll=fetch_assoc(query("SELECT * FROM `user` WHERE `id` = '$collisions[$i]' LIMIT 1"));
...
if (user_access('adm_ref') && ($ank['level']<$user['level'] || $user['id']==$ank['id']) && result(query("SELECT COUNT(*) FROM `user_ref` WHERE `id_user` = '$ank[id]'"), 0))
{
$q=query("SELECT * FROM `user_ref` WHERE `id_user` = '$ank[id]' ORDER BY `time` DESC LIMIT $set[p_str]");
echo "Посещаемые сайты:<br />\n";
while ($url=fetch_assoc($q)) {
