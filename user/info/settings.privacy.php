...
$userSet = f_array(query("SELECT * FROM `user_set` WHERE `id_user` = '".$user['id']."' LIMIT 1"));
...
{
query("UPDATE `user_set` SET `privat_str` = '".intval($_POST['privat_str'])."' WHERE `id_user` = '$user[id]'");
}

 // Сообщения
if (isset($_POST['privat_mail']) && ($_POST['privat_mail']==0 || $_POST['privat_mail']==1 || $_POST['privat_mail']==2))
{
query("UPDATE `user_set` SET `privat_mail` = '".intval($_POST['privat_mail'])."' WHERE `id_user` = '$user[id]'");
}
