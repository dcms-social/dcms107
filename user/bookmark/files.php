...
query("DELETE FROM `bookmarks` WHERE `id` = '" . intval($_GET['delete']) . "' AND `id_user` = '$user[id]' AND `type`='file' LIMIT 1");
...
$k_post=result(query("SELECT COUNT(id_file) FROM `bookmarks` WHERE `id_user` = '$ank[id]' AND `type`='file'"),0);
...
$q=query("SELECT id_file,id FROM `bookmarks`  WHERE `id_user` = '$ank[id]' AND `type`='file' ORDER BY id DESC LIMIT $start, $set[p_str]");
...
/*---------------------------*/
	...
	$file_id = fetch_assoc(query("SELECT id_dir,id,name,ras  FROM `obmennik_files` WHERE `id` = '" . $f . "'  LIMIT 1"));
