...
query("DELETE FROM `bookmarks` WHERE `id` = '" . intval($_GET['delete']) . "' AND `id_user` = '$user[id]' AND `type`='foto' LIMIT 1");
...
$k_post=result(query("SELECT COUNT(*) FROM `bookmarks` WHERE `id_user` = '$ank[id]' AND `type`='foto' "),0);
...
$q=query("SELECT * FROM `bookmarks`  WHERE `id_user` = '$ank[id]' AND `type`='foto' ORDER BY id DESC LIMIT $start, $set[p_str]");
...
$foto = fetch_assoc(query("SELECT * FROM `gallery_foto` WHERE `id` = '" . $f . "'  LIMIT 1"));
