...
{
	query("DELETE FROM `my_guests` WHERE `id_ank` = '$user[id]'");
...
$k_post = result(query("SELECT COUNT(*) FROM `my_guests` WHERE `id_ank` = '$user[id]'"),0);
...
$q = query("SELECT * FROM `my_guests` WHERE `id_ank` = '$user[id]' ORDER BY `id` DESC  LIMIT $start, $set[p_str]");
...
while ($post = f_array($q))
{
	...
	query("UPDATE `my_guests` SET `read` = '0' WHERE `id` = '$post[id]' LIMIT 1");
}
