...
{
	$folder = fetch_assoc(query("SELECT * FROM `user_files`  WHERE `id` = '".intval($_GET['edit_folder'])."' LIMIT 1"));
	...
	if (isset($_POST['name']) && isset($user))
	{
		...
		if(!isset($err))
		{
			query("UPDATE `user_files` SET `name` = '" . my_esc($name) . "',  `pass` = '" . my_esc($pass) . "', `msg` = '" . my_esc($msg) . "' WHERE `id` = '$folder[id]' LIMIT 1");
