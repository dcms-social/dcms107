...
if (isset($_GET['go']) && result(query("SELECT COUNT(*) FROM `obmennik_files` WHERE `id` = '".intval($_GET['go'])."'"),0)==1)
{
	$file_go = fetch_assoc(query("SELECT * FROM `obmennik_files` WHERE `id` = '".intval($_GET['go'])."'"));
	if (isset($_GET['ok']) && isset($_GET['ok']) && $ank['id'] == $user['id'])
	{
		query("UPDATE `obmennik_files` SET `my_dir` = '$dir[id]' WHERE `id` = '$file_go[id]' LIMIT 1");
...
{
	if (!isset($_SESSION['obmen_dir']) && result(query("SELECT COUNT(*) FROM `obmennik_dir` WHERE `id` = '" . intval($_GET['obmen_dir']) . "' AND `upload` = '1'"),0) == 1)
...
$k_files=result(query("SELECT COUNT(*) FROM `obmennik_files`  WHERE `my_dir` = '$dir[id]' AND `id_user` = '$ank[id]'"),0);
$k_post=result(query("SELECT COUNT(*) FROM `user_files` WHERE `id_dir` = '$dir[id]' AND `id_user` = '$ank[id]'"),0);
...
$q=query("SELECT * FROM `user_files`  WHERE `id_dir` = '$dir[id]'  AND `id_user` = '$ank[id]' ORDER BY time DESC LIMIT $start, $set[p_str]");
...
/*----------------------Счетчик папок---------------------*/
$k_f=0;
$q3=query("SELECT * FROM `user_files` WHERE `id_dires` like '%$post[id]%'");
while ($post2 = fetch_assoc($q3))
{
$k_f=$k_f+result(query("SELECT COUNT(*) FROM `user_files` WHERE `id_dir` = '$post2[id]'"),0);
}
$k_f=$k_f+result(query("SELECT COUNT(*) FROM `user_files` WHERE `id_dir` = '$post[id]'"),0);
/*---------------------------------------------------------*/

/*----------------------Счетчик файлов--------------------*/
$k_f2=0;
$q4=query("SELECT * FROM `user_files` WHERE `id_dires` like '%$post[id]%'");
while ($post3 = fetch_assoc($q4))
{
$k_f2=$k_f2+result(query("SELECT COUNT(*) FROM `obmennik_files` WHERE `my_dir` = '$post3[id]'"),0);
}
$k_f2=$k_f2+result(query("SELECT COUNT(*) FROM `obmennik_files` WHERE `my_dir` = '$post[id]'"),0);
/*---------------------------------------------------------*/
echo ' ('.$k_f.'/'.$k_f2.') ';
...
if (!isset($_GET['go']))
{
$q2=query("SELECT * FROM `obmennik_files`  WHERE `my_dir` = '$dir[id]' AND `id_user` = '$ank[id]' ORDER BY time DESC LIMIT $start, $set[p_str]");
//echo "<form method='post' action='?move_file'>";

while ($post = fetch_assoc($q2))
{
$k_p=result(query("SELECT COUNT(*) FROM `obmennik_komm` WHERE `id_file` = '$post[id]'"),0);
$dir_id=fetch_assoc(query("SELECT * FROM `obmennik_dir` WHERE `id` = '$post[id_dir]' LIMIT 1"));
$ras=$post['ras'];
