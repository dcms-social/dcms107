...
{
    echo "<div class='webmenu last'>";
    echo "<a href='new.php' class='activ'>Заявки (".result(query("SELECT COUNT(id) FROM `frends_new` WHERE `to` = '$ank[id]' LIMIT 1"), 0).")</a>";
    echo "</div>"; 
}
echo "</div>";
}
...
$k_post=result(query("SELECT COUNT(id) FROM `frends_new` WHERE `to` = '$ank[id]' LIMIT 1"), 0);
...
$q = query("SELECT * FROM `frends_new` WHERE `to` = '$user[id]' ORDER BY time DESC");
