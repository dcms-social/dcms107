...
$frend = fetch_assoc(query("SELECT * FROM `frends` WHERE `user` = '" . $user['id'] . "' AND `frend` = '$ank[id]' AND `i` = '1'", $db));
...
if (isset($_POST['save']))
{
	...
	if (isset($_POST['disc_foto']) && ($_POST['disc_foto'] == 0 || $_POST['disc_foto'] == 1))
	{
		...
		query("UPDATE `frends` SET `disc_foto` = '" . $disc . "' WHERE `user` = '$user[id]' AND `frend` = '$ank[id]'");
	}
	...
	if (isset($_POST['disc_obmen']) && ($_POST['disc_obmen'] == 0 || $_POST['disc_obmen'] == 1))
	{
		...
		query("UPDATE `frends` SET `disc_obmen` = '" . $disc . "' WHERE `user` = '$user[id]' AND `frend` = '$ank[id]'");
	}
	...
	if (isset($_POST['disc_status']) && ($_POST['disc_status'] == 0 || $_POST['disc_status'] == 1))
	{
		...
		query("UPDATE `frends` SET `disc_status` = '" . $disc . "' WHERE `user` = '$user[id]' AND `frend` = '$ank[id]'");
	}
	...
	if (isset($_POST['disc_notes']) && ($_POST['disc_notes'] == 0 || $_POST['disc_notes'] == 1))
	{
		...
		query("UPDATE `frends` SET `disc_notes` = '" . $disc . "' WHERE `user` = '$user[id]' AND `frend` = '$ank[id]'");
	}
	...
	if (isset($_POST['disc_forum']) && ($_POST['disc_forum'] == 0 || $_POST['disc_forum'] == 1))
	{
		...
		query("UPDATE `frends` SET `disc_forum` = '" . $disc . "' WHERE `user` = '$user[id]' AND `frend` = '$ank[id]'");
	}
