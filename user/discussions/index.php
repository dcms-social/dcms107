...
if (isset($_GET['read']) && $_GET['read'] == 'all')
{
	if (isset($user))
	{
		query("UPDATE `discussions` SET `count` = '0' WHERE `id_user` = '$user[id]'");
...
if (isset($_GET['delete']) && $_GET['delete']=='all')
{
	if (isset($user))
	{
		query("DELETE FROM `discussions` WHERE `id_user` = '$user[id]'");
...
if (isset($_GET['likestatus']))
{
	...
	result(query("SELECT COUNT(*) FROM `status_like` WHERE `id_status` = '$status[id]' AND `id_user` = '$user[id]' LIMIT 1"),0) == 0)
	{
		...
		{
			$a = get_user($f['frend']);
			query("INSERT INTO `tape` (`id_user`,`ot_kogo`,  `avtor`, `type`, `time`, `id_file`) 
...
if (result(query("SELECT COUNT(*) FROM `discussions`  WHERE `id_user` = '$user[id]' AND `count` > '0' AND `avtor` = '$user[id]'"),0) > 0)
...
// Уведомления
$k_notif = result(query("SELECT COUNT(`read`) FROM `notification` WHERE `id_user` = '$user[id]' AND `read` = '0'"), 0);

if ($k_notif > 0)$k_notif = '<font color=red>(' . $k_notif . ')</font>';
else $k_notif = null;

// Обсуждения
$discuss = result(query("SELECT COUNT(`count`) FROM `discussions` WHERE `id_user` = '$user[id]' AND `count` > '0' "),0);

if ($discuss > 0)$discuss = '<font color=red>(' . $discuss . ')</font>';
else $discuss = null;

// Лента
$lenta = result(query("SELECT COUNT(`read`) FROM `tape` WHERE `id_user` = '$user[id]' AND `read` = '0' "),0);
...
<?
$k_post = result(query("SELECT COUNT(*) FROM `discussions`  WHERE `id_user` = '$user[id]' $order"),0);
$k_page = k_page($k_post, $set['p_str']);
$page = page($k_page);
$start = $set['p_str'] * $page - $set['p_str'];

$q = query("SELECT * FROM `discussions` WHERE `id_user` = '$user[id]' $order ORDER BY `time` DESC LIMIT $start, $set[p_str]");
