...
if (isset($_GET['id']) && is_numeric($_GET['id']) && result(query("SELECT COUNT(`id`) FROM `stena` WHERE `id` = '".intval($_GET['id'])."' LIMIT 1"),0)!=0)
{
...
$mat = antimat($msg);
     ...
     elseif (result(query("SELECT COUNT(`id`) FROM `stena_komm` WHERE `id_user` = '" . $user['id'] . "' AND `msg` = '" . my_esc($msg) . "' AND `id_stena`='$id' LIMIT 1"), 0) != 0)
     {
     $err[] = 'Ваше сообщение повторяет предыдущее';
     }
     elseif(!isset($err))
     {
     query("INSERT INTO `stena_komm` (`id_user`, `time`, `msg`,`id_stena`) values('" . $user['id'] . "', '" . $time . "', '" . my_esc($msg) . "','".$id."')");
     ...
     */
if (isset($user)){
		$notifiacation=fetch_assoc(query("SELECT * FROM `notification_set` WHERE `id_user` = '".$post['id_user']."' LIMIT 1"));
			
			if ($notifiacation['komm'] == 1 && $post['id_user'] != $user['id'])
			query("INSERT INTO `notification` (`avtor`, `id_user`, `id_object`, `type`, `time`) VALUES ('$user[id]', '$post[id_user]', '$post[id]', 'stena_komm2', '$time')");
...
elseif (isset($_GET['del']) && result(query("SELECT COUNT(*) FROM `stena_komm` WHERE `id` = '".intval($_GET['del'])."' AND `id_stena` = '$post[id]'"),0))
{
if (isset($user) && ($user['level']>=3 || $user['id']=$stena['id_user']))
{
query("DELETE FROM `stena_komm` WHERE `id` = '".intval($_GET['del'])."' LIMIT 1");
...
$post=fetch_assoc(query("SELECT * FROM `stena` WHERE `id` = '".abs(intval($_GET['id']))."' LIMIT 1"));
echo "  <div class='nav2'>\n";
...
$k_post=result(query("SELECT COUNT(*) FROM `stena_komm` WHERE `id_stena` = '$id'"),0);
$k_page=k_page($k_post,$set['p_str']);
$page=page($k_page);
$start=$set['p_str']*$page-$set['p_str'];
$q=query("SELECT * FROM `stena_komm` WHERE `id_stena` = '".intval($_GET['id'])."' ORDER BY `id` DESC LIMIT $start, $set[p_str]");
