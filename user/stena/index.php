...
$stena=fetch_assoc(query("SELECT * FROM `stena` WHERE `id` = '".intval($_GET['likepost'])."' LIMIT 1"));
...
$l=result(query("SELECT COUNT(*) FROM `stena_like` WHERE `id_stena` = '$stena[id]'"),0);




if (isset($_GET['likepost']) && result(query("SELECT COUNT(*) FROM `stena_like` WHERE
...
query("INSERT INTO `stena_like` (`id_user`, `id_stena`) values('$user[id]', '$stena[id]')");




query("UPDATE `user` SET `balls` = '".($ank3['balls']+1)."' WHERE `id` = '$ank3[id]' LIMIT 1");
...
$k_post=result(query("SELECT COUNT(*) FROM `stena` WHERE `id_stena` = '$ank[id]'"),0);
...
$q=query("SELECT * FROM `stena` WHERE `id_stena` = '$ank[id]' ORDER BY id $sort LIMIT $start, $set[p_str]");
...
$ank_stena=fetch_assoc(query("SELECT * FROM `user` WHERE `id` = $post[id_user] LIMIT 1"));
...
$l=result(query("SELECT COUNT(*) FROM `stena_like` WHERE `id_stena` = '$post[id]'"),0);




echo '<a href="/user/komm.php?id='.$post['id'].'"><img src="/style/icons/uv.png"> ('.result(query("SELECT COUNT(*) FROM `stena_komm` WHERE `id_stena` = '$post[id]'"),0).') </a><span style="float:right;"> <a href="?id='.$ank['id'].'&amp;likepost='.$post['id'].'&amp;page='.$page.'" >&hearts; '.$l.'</a> ';
