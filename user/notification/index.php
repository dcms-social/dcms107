...
if (isset($user))
{
	if (result(query("SELECT COUNT(*) FROM `notification`  WHERE `id_user` = '$user[id]' AND `id` = '".intval($_GET['del'])."'"),0)==1)
	{
	query("DELETE FROM `notification` WHERE `id_user` = '$user[id]' AND `id` = '".intval($_GET['del'])."' LIMIT 1");
...
$discuss = result(query("SELECT COUNT(`count`) FROM `discussions` WHERE `id_user` = '$user[id]' AND `count` > '0' "),0); // Обсуждения
if ($discuss > 0)$discuss = '<font color=red>('.$discuss.')</font>';
else $discuss = null;

$lenta = result(query("SELECT COUNT(`read`) FROM `tape` WHERE `id_user` = '$user[id]' AND `read` = '0' "),0); // Лента
...
==========
*/
	$k_post=result(query("SELECT COUNT(*) FROM `notification`  WHERE `id_user` = '$user[id]' "),0);
...
{	
	...
	$status = fetch_assoc(query("SELECT * FROM `status` WHERE `id` = '".$post['id_object']."' LIMIT 1"));
...
{	
if ($type == 'new_gift'){
	$id_gift =  fetch_assoc(query("SELECT id,id_gift FROM `gifts_user` WHERE `id` = '$post[id_object]' LIMIT 1"));
	$gift =  fetch_assoc(query("SELECT * FROM `gift_list` WHERE `id` = '$id_gift[id_gift]' LIMIT 1"));
}else{
	$gift =  fetch_assoc(query("SELECT * FROM `gift_list` WHERE `id` = '$post[id_object]' LIMIT 1"));
}
if ($avtor['id']){
	echo  group($avtor['id'])." ";
...
if ($post['read'] == 0) query("UPDATE `notification` SET `read` = '1' WHERE `id` = '$post[id]'");
...
if ($avtor['id']){
	...
	echo "<div style='text-align:right;'><a href='?komm&amp;del=$post[id]&amp;page=$page'><img src='/style/icons/delete.gif' alt='*' /></a></div>";
	query("UPDATE `notification` SET `read` = '1' WHERE `id` = '$post[id]'");
} 
/*
===============================
Дневники коментарии
...
{	
if ($post['read'] == 0) query("UPDATE `notification` SET `read` = '1' WHERE `id` = '$post[id]'");
	$notes = fetch_assoc(query("SELECT * FROM `notes` WHERE `id` = '".$post['id_object']."' LIMIT 1"));
if ($notes['id']){
...
{	if ($post['read'] == 0) query("UPDATE `notification` SET `read` = '1' WHERE `id` = '$post[id]'");
	$file = fetch_assoc(query("SELECT * FROM `obmennik_files` WHERE `id` = '".$post['id_object']."' LIMIT 1"));
	$dir=fetch_assoc(query("SELECT * FROM `user_files` WHERE `id` = '".$file['my_dir']."' LIMIT 1"));
	$ras = $file['ras'];
if ($file['id'] && $avtor['id']){
	echo status($avtor['id']) .  group($avtor['id']) . " <a href='/info.php?id=$avtor[id]'>$avtor[nick]</a>  " . medal($avtor['id']) . " " . online($avtor['id']) . " $name ";
...
{	
if ($post['read'] == 0) query("UPDATE `notification` SET `read` = '1' WHERE `id` = '$post[id]'");
	$foto = fetch_assoc(query("SELECT * FROM `gallery_foto` WHERE `id` = '".$post['id_object']."' LIMIT 1"));
if ($foto['id']){
	echo status($avtor['id']) .  group($avtor['id']) . " <a href='/info.php?id=$avtor[id]'>$avtor[nick]</a>  " . medal($avtor['id']) . " " . online($avtor['id']) . " $name ";
...
if ($type == 'them_komm') 
{	
	$them=fetch_assoc(query("SELECT * FROM `forum_t` WHERE `id` = '".$post['id_object']."' LIMIT 1"));
if ($post['read'] == 0) query("UPDATE `notification` SET `read` = '1' WHERE `id` = '$post[id]'");
if ($them['id']){
	echo status($avtor['id']) .  group($avtor['id']) . " <a href='/info.php?id=$avtor[id]'>$avtor[nick]</a>  " . medal($avtor['id']) . " " . online($avtor['id']) . " $name ";
...
if ($type == 'news_komm') 
{	if ($post['read'] == 0) query("UPDATE `notification` SET `read` = '1' WHERE `id` = '$post[id]'");
	$news = fetch_assoc(query("SELECT * FROM `news` WHERE `id` = '".$post['id_object']."' LIMIT 1"));
	echo status($avtor['id']) .  group($avtor['id']) . " <a href='/info.php?id=$avtor[id]'>$avtor[nick]</a>  " . medal($avtor['id']) . " " . online($avtor['id']) . " $name ";
