...
$k_post=result(query("SELECT COUNT(*) FROM `user`"),0);
...
$q=query("SELECT `user`.`id` FROM `user` LEFT JOIN `user_group` ON `user`.`group_access` = `user_group`.`id` ORDER BY $sql_sort $por LIMIT $start, $set[p_str]");
...
$k_post=result(query("SELECT COUNT(*) FROM `user` WHERE `nick` like '%".mysql_real_escape_string($usearch)."%' OR `id` = '".intval($usearch)."'"),0);
...
$q=query("SELECT `id` FROM `user` WHERE `nick` like '%".mysql_real_escape_string($usearch)."%' OR `id` = '".intval($usearch)."' ORDER BY `$sort` $por LIMIT $start, $set[p_str]");
