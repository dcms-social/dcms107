...
{
if (result(query("SELECT COUNT(*) FROM `user` WHERE `type_reg` = '" . $users['network'] . "' AND `identity` = '" . $users['identity'] . "'"),0) == 0)
{
/*
================================
Имя к id и пол
...
================================
*/
if (result(query("SELECT COUNT(*) FROM `user` WHERE `nick` = '" . $identity . "'"),0)!=0)
...
query("INSERT INTO `user` (`set_nick`, `nick`, `pass`, `date_reg`, `date_last`, `pol`, `ank_city`, `ank_name`, `identity`, `type_reg`)
 values('1', '$identity', '" . shif($pass) . "', '$time', '$time', '" . $pol . "', '" . $users['city'] . "', '" . $users['first_name'] . "', '" . $users['identity'] . "', '" . $users['network'] . "')",$db);

$user=fetch_assoc(query("SELECT * FROM `user` WHERE `nick` = '". $identity ."' AND `pass` = '". shif($pass) ."' LIMIT 1"));// отправка сообщения
...
query("INSERT INTO `mail` (`id_user`, `id_kont`, `msg`, `time`) values('0', '$user[id]', '".my_esc($msgg)."', '$time')");
...
query("INSERT INTO `gallery` (`id_user`, `name`) values('$user[id]', 'Личные фото')");

$gallery=fetch_assoc(query("SELECT * FROM `gallery` WHERE `id_user` = '$user[id]'  LIMIT 1"));
...
if (!isset($err)){
query("INSERT INTO `gallery_foto` (`id_gallery`, `name`, `ras`, `type`, `id_user`,`avatar`) values ('$gallery[id]', '$name', 'jpg', 'image/jpeg', '$user[id]','1')");
...
query("UPDATE `gallery_foto` SET `avatar` = '1' WHERE `id` = '$id_foto' LIMIT 1");
...
exit;
	...
	$user=fetch_assoc(query("SELECT * FROM `user` WHERE `type_reg` = '" . $users['network'] . "' AND `identity` = '" . $users['identity'] . "' LIMIT 1"));
