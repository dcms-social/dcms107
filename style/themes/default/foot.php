...
<div class="foot">
На сайте: 
<a href="/online.php"><?=result(query("SELECT COUNT(*) FROM `user` WHERE `date_last` > ".(time()-600).""), 0)?></a> &amp;
<a href="/online_g.php"><?=result(query("SELECT COUNT(*) FROM `guests` WHERE `date_last` > ".(time()-600)." AND `pereh` > '0'"), 0)?></a>
<?
if (!$set['web'])
