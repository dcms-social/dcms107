...
while ($post_menu = fetch_assoc($q_menu))
...
if (!isset($post_menu['icon']))query('ALTER TABLE `menu` ADD `icon` VARCHAR( 32 ) NULL DEFAULT NULL');



if (!isset($post_menu['type']))query("ALTER TABLE  `menu` ADD  `type` ENUM('link', 'razd') NOT NULL DEFAULT 'link' AFTER `id`");
