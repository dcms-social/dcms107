...
<table>
    <div id="footer" class="gradient_grey">
        <div class="body_width_limit">
            <span id="copyright">
<a href="/user/users.php">Зарегистрировано (<?=result(query("SELECT COUNT(`id`)FROM `user`"),0);?>)</a>
            </span>
            <span id="copyright">
<a href="/online.php">Онлайн (<?=result(query("SELECT COUNT(`id`) FROM `user` WHERE `date_last` > " . (time() - 600) . ""), 0);?>)</a>
            </span>
            <span id="copyright">
<a href="/online_g.php">Гостей (<?=result(query("SELECT COUNT(*) FROM `guests` WHERE `date_last` > " . (time() - 600) . " AND `pereh` > '0'"), 0);?>)</a>
