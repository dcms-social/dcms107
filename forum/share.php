...
if (isset($user) && result(query("SELECT COUNT(`id`) FROM `ban` WHERE `razdel` = 'forum' AND `id_user` = '$user[id]' AND (`time` > '$time' OR `view` = '0' OR `navsegda` = '1')"), 0)!=0)
{
...
title();
aut();
$not=query("SELECT * FROM `forum_t` WHERE `id`='".intval($_GET['id'])."' LIMIT 1");
if(rows($not)==0){
echo "<div class='error'>Такой темы не существует</div>";
include_once '../sys/inc/tfoot.php';
exit;
}
if(result(query("SELECT COUNT(`id`)FROM `notes` WHERE `id_user`='".$user['id']."' AND `share_id`='".intval($_GET['id'])."' AND `share_type`='forum' LIMIT 1"),0)==1){
echo "<div class='error'>Вы уже поделились данной темой</div>";
include_once '../sys/inc/tfoot.php';
exit;
}else{
$notes=fetch_assoc($not);
$avtor=get_user($notes['id_user']);
if(isset($_POST['ok'])){
query("INSERT INTO `notes`(`id_user`,`name`,`msg`,`share`,`share_text`,`share_id`,`share_id_user`,`share_name`,`time`,`share_type`) values('".$user['id']."','".text($notes['name'])."','".my_esc($_POST['share_text'])."','1','".my_esc($notes['text'])."','".$notes['id']."','".$notes['id_user']."','".my_esc($notes['name'])."','".$time."','forum')");
$id=mysql_insert_id();
msg('Ок всё крч');
