...
$q=query("SELECT * FROM `forum_p` WHERE `id_them` = '$them[id]' AND `id_forum` = '$forum[id]' AND `id_razdel` = '$razdel[id]' ORDER BY `time` ASC");
...
if ($post['cit']!=NULL && result(query("SELECT COUNT(*) FROM `forum_p` WHERE `id` = '$post[cit]'"),0)==1)

{

$cit=fetch_assoc(query("SELECT * FROM `forum_p` WHERE `id` = '$post[cit]' LIMIT 1"));
...
if(result(query("SELECT COUNT(*) FROM `bookmarks` WHERE `id_user` = $user[id] AND `type`='forum' AND `id_object` = '$them[id]'"),0)!=0)
...
query("INSERT INTO `bookmarks` (`id_user`, `time`,  `id_object`, `type`) values('$user[id]', '$time', '$them[id]', 'forum')");
...
query("DELETE FROM `bookmarks` WHERE `id_user` = '$user[id]' AND `type`='forum' AND `id_object` = '$them[id]'");
...
if (isset($user) && ($them['close'] == 0 || $them['close'] == 1 && user_access('forum_post_close')) && isset($_GET['act']) && $_GET['act'] == 'new' && isset($_POST['msg']) && !isset($_POST['file_s'])) {
    ...
    if (result(query("SELECT COUNT(*) FROM `forum_p` WHERE `id_them` = '$them[id]' AND `id_forum` = '$forum[id]' AND `id_razdel` = '$razdel[id]' AND `id_user` = '$user[id]' AND `msg` = '" . my_esc($msg) . "' LIMIT 1"), 0) != 0)
    ...
    if (!isset($err)) {
        ...
        if (isset($_POST['cit']) && is_numeric($_POST['cit']) && result(query("SELECT COUNT(*) FROM `forum_p` WHERE `id` = '" . intval($_POST['cit']) . "' AND `id_them` = '" . intval($_GET['id_them']) . "' AND `id_razdel` = '" . intval($_GET['id_razdel']) . "' AND `id_forum` = '" . intval($_GET['id_forum']) . "'"), 0) == 1)
            $cit = intval($_POST['cit']);
        else
            $cit = 'null';

        query("UPDATE `user` SET `balls` = '" . ($user['balls'] + 1) . "' WHERE `id` = '$user[id]' LIMIT 1");

        query("UPDATE `forum_zakl` SET `time_obn` = '$time' WHERE `id_them` = '$them[id]'");

        query("INSERT INTO `forum_p` (`id_forum`, `id_razdel`, `id_them`, `id_user`, `msg`, `time`, `cit`) values('$forum[id]', '$razdel[id]', '$them[id]', '$user[id]', '" . my_esc($msg) . "', '$time', $cit)");
        ...
        if (isset($_SESSION['file']) && isset($user)) {
            ...
            for ($i = 0; $i < count($_SESSION['file']); $i++) {
                ...
                if (isset($_SESSION['file'][$i]) && is_file($_SESSION['file'][$i]['tmp_name'])) {
                    ...
                    query("INSERT INTO `forum_files` (`id_post`, `name`, `ras`, `size`, `type`) values('$post_id', '" . $_SESSION['file'][$i]['name'] . "', '" . $_SESSION['file'][$i]['ras'] . "', '" . $_SESSION['file'][$i]['size'] . "', '" . $_SESSION['file'][$i]['type'] . "')");
        ...
        query("UPDATE `forum_r` SET `time` = '$time' WHERE `id` = '$razdel[id]' LIMIT 1");
     /*
        ...
        $q = query("SELECT * FROM `frends` WHERE `user` = '" . $them['id_user'] . "' AND `i` = '1'");

        while ($f = f_array($q)) {

            $a = get_user($f['frend']);

            $discSet = f_array(query("SELECT * FROM `discussions_set` WHERE `id_user` = '" . $a['id'] . "' LIMIT 1")); // Общая настройка обсуждений
            ...
            if ($f['disc_forum'] == 1 && $discSet['disc_forum'] == 1) /* Фильтр рассылки */ {
                ...
                if (result(query("SELECT COUNT(*) FROM `discussions` WHERE `id_user` = '$a[id]' AND `type` = 'them' AND `id_sim` = '$them[id]' LIMIT 1"), 0) == 0) {

                    if ($them['id_user'] != $a['id'] || $a['id'] != $user['id'])
                        query("INSERT INTO `discussions` (`id_user`, `avtor`, `type`, `time`, `id_sim`, `count`) values('$a[id]', '$them[id_user]', 'them', '$time', '$them[id]', '1')");
                }

                else {

                    $disc = f_array(query("SELECT * FROM `discussions` WHERE `id_user` = '$a[id_user]' AND `type` = 'them' AND `id_sim` = '$them[id]' LIMIT 1"));

                    if ($them['id_user'] != $a['id'] || $a['id'] != $user['id'])
                        query("UPDATE `discussions` SET `count` = '" . ($disc['count'] + 1) . "', `time` = '$time' WHERE `id_user` = '$a[id]' AND `type` = 'them' AND `id_sim` = '$them[id]' LIMIT 1");
                }
            }
        }
...
// отправляем автору
        ...
        if (result(query("SELECT COUNT(*) FROM `discussions` WHERE `id_user` = '$them[id_user]' AND `type` = 'them' AND `id_sim` = '$them[id]' LIMIT 1"), 0) == 0) {

            if ($them['id_user'] != $user['id'] && $them['id_user'] != $ank_otv['id'])
                query("INSERT INTO `discussions` (`id_user`, `avtor`, `type`, `time`, `id_sim`, `count`) values('$them[id_user]', '$them[id_user]', 'them', '$time', '$them[id]', '1')");
        }

        else {

            $disc = f_array(query("SELECT * FROM `discussions` WHERE `id_user` = '$them[id_user]' AND `type` = 'them' AND `id_sim` = '$them[id]' LIMIT 1"));

            if ($them['id_user'] != $user['id'] && $them['id_user'] != $ank_otv['id'])
                query("UPDATE `discussions` SET `count` = '" . ($disc['count'] + 1) . "', `time` = '$time' WHERE `id_user` = '$them[id_user]' AND `type` = 'them' AND `id_sim` = '$them[id]' LIMIT 1");
        }
...
if (isset($_POST['cit'])) 
{
$cit2=fetch_assoc(query("SELECT * FROM `forum_p` WHERE `id` = '$cit' LIMIT 1"));
...
$ank_otv['id'] = $cit2['id_user'];
            ...
            $notifiacation = fetch_assoc(query("SELECT * FROM `notification_set` WHERE `id_user` = '" . $ank_otv['id'] . "' LIMIT 1"));
    


            if ($notifiacation['komm'] == 1)
                query("INSERT INTO `notification` (`avtor`, `id_user`, `id_object`, `type`, `time`) VALUES ('$user[id]', '$ank_otv[id]', '$them[id]', 'them_komm', '$time')");
        }
...
$mess = fetch_assoc(query("SELECT * FROM `forum_p` WHERE `id` = '".intval($_GET['spam'])."' limit 1"));

$spamer = get_user($mess['id_user']);

if (result(query("SELECT COUNT(*) FROM `spamus` WHERE `id_user` = '$user[id]' AND `id_spam` = '$spamer[id]' AND `razdel` = 'forum' AND `spam` = '".$mess['msg']."'"),0)==0)
...
query("INSERT INTO `spamus` (`id_object`, `id_user`, `msg`, `id_spam`, `time`, `types`, `razdel`, `spam`) values('$them[id]', '$user[id]', '$msg', '$spamer[id]', '$time', '$types', 'forum', '".my_esc($mess['msg'])."')");
...
if (result(query("SELECT COUNT(*) FROM `spamus` WHERE `id_user` = '$user[id]' AND `id_spam` = '$spamer[id]' AND `razdel` = 'forum'"),0)==0)
...
result(query("SELECT COUNT(*) FROM `forum_files` WHERE `id` = '".intval($_GET['id_file'])."'"), 0)==1

&&

 result(query("SELECT COUNT(*) FROM `forum_files_rating` WHERE `id_user` = '$user[id]' AND `id_file` = '".intval($_GET['id_file'])."'"), 0)==0)
...
query("INSERT INTO `forum_files_rating` (`id_user`, `id_file`, `rating`) values('$user[id]', '".intval($_GET['id_file'])."', '-1')");
...
msg ('Ваш положительный отзыв принят');
	...
	$k_post=result(query("SELECT COUNT(*) FROM `forum_p` WHERE `id_them` = '$them[id]' AND `id_forum` = '$forum[id]' AND `id_razdel` = '$razdel[id]'"),0);
...
$q = query("SELECT * FROM `forum_f` ORDER BY `pos` ASC");

while ($forums = fetch_assoc($q))

{

echo "<optgroup label='$forums[name]'>\n";

$q2 = query("SELECT * FROM `forum_r` WHERE `id_forum` = '$forums[id]' ORDER BY `time` DESC");

while ($razdels = fetch_assoc($q2))
...
$q2 = query("SELECT * FROM `forum_r` WHERE `id_forum` = '$forum[id]' ORDER BY `time` DESC");
...
exit;
...

		{
			...
			query("DELETE FROM `forum_p` WHERE `id` = '" . intval($_GET['del']) . "' LIMIT 1");
...
if (isset($_GET['act']) && $_GET['act'] == 'vote' && (user_access('forum_them_edit') || $ank2['id'] == $user['id'])) {
  if (result(query("SELECT COUNT(`id`) FROM `votes_forum` WHERE `them` = '".abs(intval($them['id']))."' LIMIT 1"),0)!=0){
 if(isset($_POST['del']) && isset($user)){ 
 query("UPDATE `forum_t` SET `vote`='',`vote_time`='',`vote_close` ='0' WHERE `id` = '$them[id]' LIMIT 1");
 query("DELETE FROM `votes_forum` WHERE `them` = '$them[id]'  ");
 query("DELETE FROM `votes_user` WHERE `them` = '$them[id]'  ");
...
if(!isset($err)){ 
query("UPDATE `forum_t` SET `vote`='$text',`vote_close` ='$close' WHERE `id` = '$them[id]' LIMIT 1");
} 
for ($x=1; $x<7; $x++){ 
...
if(!isset($err)){ 
query("UPDATE `votes_forum` SET `var`='$add' WHERE `num` = '$x' LIMIT 1");
...
$q=query("SELECT * FROM `votes_forum` WHERE `them` = '".abs(intval($them['id']))."' ORDER BY `id` ASC  LIMIT 6");
...
if(!isset($err)){ 
query("UPDATE `forum_t` SET `vote`='$text',`vote_close` ='0' WHERE `id` = '$them[id]' LIMIT 1");
} 
for ($x=1; $x<7; $x++){ 
...
query("INSERT INTO `votes_forum` (`them`,`var`,`num`) values('$them[id]','$add','$x')");
...
if (isset($_GET['vote_user']) && result(query("SELECT * FROM `votes_user` WHERE `var` = '".intval($_GET['vote_user'])."' AND `them`='$them[id]' "),0)!=0 ) {
$us=intval($_GET['vote_user']);  

$k_post = result(query("SELECT * FROM `votes_user` WHERE  `var` = '$us' AND `them`='$them[id]'"),0);
$k_page=k_page($k_post,$set['p_str']); 
$page=page($k_page); 
$start=$set['p_str']*$page-$set['p_str']; 

$q=query("SELECT `id_user`,`time` FROM `votes_user` WHERE  `var` = '$us' AND `them`='$them[id]' ORDER BY `time`  LIMIT $start, $set[p_str] ");
while ($row = fetch_assoc($q)){
...
if (result(query("SELECT * FROM `votes_user` WHERE `them` = '".abs(intval($them['id']))."'  AND `id_user`='$user[id]' LIMIT 1"),0)==0  && $them['vote_close']!='1' && $them['close']=='0'){
 query("INSERT INTO `votes_user` (`them`,`id_user`,`var`,`time`) values('$them[id]','$user[id]','$vote','".time()."')");
...
==========
*/
$vote_c=result(query("SELECT COUNT(*) FROM `votes_forum` WHERE `them` = '".abs(intval($them['id']))."' LIMIT 1"),0);
 if ($vote_c!=0){ 
?><div class="round_corners poll_block stnd_padd"><div style="font-size:14px;">Опрос: <b><?=output_text($them['vote']);?></b></div><?php

$q=query("SELECT * FROM `votes_forum` WHERE `them` = '".abs(intval($them['id']))."' AND `var` != '' LIMIT 6");
?><form action="" method="post"><?php 
while ($row = fetch_assoc($q)){

$sum=result(query("SELECT COUNT(*) FROM `votes_user` WHERE `them` = '$row[them]'"), 0);
$var=result(query("SELECT COUNT(*) FROM `votes_user` WHERE `them` = '$row[them]' AND `var` = '$row[num]'"), 0);

 if($sum==0)$poll=0; 
elseif($var==0)$poll=0; 
else $poll=($var/$sum)*100; 
$us=result(query("SELECT COUNT(*) FROM `votes_user` WHERE `them` = '".abs(intval($them['id']))."'  AND `id_user`='$user[id]' LIMIT 1"),0);
if($us=='0' && isset($user)){
...
echo "<div class='mess'>";
$share=result(query("SELECT COUNT(`id`)FROM `notes` WHERE `share_id`='".$them['id']."' AND `share_type`='forum'"),0);
if(result(query("SELECT COUNT(`id`)FROM `notes` WHERE `id_user`='".$user['id']."' AND `share_type`='forum' AND `share_id`='".$them['id']."' LIMIT 1"),0)==0 && isset($user)) {
...
$markinfo=result(query("SELECT COUNT(`id`) FROM `bookmarks` WHERE `id_object` = '".$them['id']."' AND `type`='forum'"),0);

echo "<br/><img src='/style/icons/add_fav.gif' alt='*' /> ";

if (result(query("SELECT COUNT(`id`) FROM `bookmarks` WHERE `id_object` = '$them[id]' AND `id_user` = '$user[id]' AND `type`='forum'"),0)==0)
...
$q=query("SELECT * FROM `forum_p` WHERE `id_them` = '$them[id]' AND `id_forum` = '$forum[id]' AND `id_razdel` = '$razdel[id]' ORDER BY `time` $sort$lim");
...
$postBan = result(query("SELECT COUNT(*) FROM `ban` WHERE (`razdel` = 'all' OR `razdel` = 'forum') AND `post` = '1' AND `id_user` = '$ank[id]' AND (`time` > '$time' OR `navsegda` = '1')"), 0);
...
$status=fetch_assoc(query("SELECT * FROM `status` WHERE `pokaz` = '1' AND `id_user` = '$ank[id]' LIMIT 1"));
