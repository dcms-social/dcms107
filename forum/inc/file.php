...
if (result(query("SELECT COUNT(*) FROM `forum_files` WHERE `id_post` = '$post[id]'"), 0)>0)
{
?>
...
<?
$q_f=query("SELECT * FROM `forum_files` WHERE `id_post` = '$post[id]'");
while ($file = fetch_assoc($q_f))
{
...
if (!isset($file['count']))query("ALTER TABLE `forum_files` ADD `count` INT DEFAULT '0' NOT NULL");

echo "<td style='text-align:center;'>$file[count]</td>\n";
echo "<td style='text-align:center;'> ";

$k_vote=result(query("SELECT COUNT(*) FROM `forum_files_rating` WHERE `id_file` = '$file[id]'"), 0);
$sum_vote=result(query("SELECT SUM(`rating`) FROM `forum_files_rating` WHERE `id_file` = '$file[id]'"), 0);

if ($sum_vote==null)$sum_vote=0;


if (isset($user) && $user['balls']>=50 && $user['rating']>=0 && result(query("SELECT COUNT(*) FROM `forum_files_rating` WHERE `id_user` = '$user[id]' AND `id_file` = '$file[id]'"), 0)==0)
