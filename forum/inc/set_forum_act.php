...
query("UPDATE `forum_f` SET `adm` = '$adm' WHERE `id` = '$forum[id]' LIMIT 1");
}
...
query("UPDATE `forum_f` SET `name` = '$name', `opis` = '$opis',`icon`='$FIcon', `pos` = '$pos' WHERE `id` = '$forum[id]' LIMIT 1");
$forum=fetch_assoc(query("SELECT * FROM `forum_f` WHERE `id` = '$forum[id]' LIMIT 1"));
...
{
query("DELETE FROM `forum_f` WHERE `id` = '$forum[id]'");
query("DELETE FROM `forum_r` WHERE `id_forum` = '$forum[id]'");
query("DELETE FROM `forum_t` WHERE `id_forum` = '$forum[id]'");
query("DELETE FROM `forum_p` WHERE `id_forum` = '$forum[id]'");
...
if (!isset($err))
{
	...
	query("INSERT INTO `forum_r` (`id_forum`, `opis`,`name`, `time`) values('$forum[id]', '".my_esc($_POST['opis'])."','".my_esc($name)."', '$time')");
