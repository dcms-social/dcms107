...
/* Бан пользователя */
if (isset($user['id'])) {
  if (result(query("SELECT COUNT(*) FROM `ban` WHERE `razdel` = 'forum' AND `id_user` = '$user[id]' AND (`time` > '$time' OR `view` = '0' OR `navsegda` = '1')"), 0) != 0) {
...
if (isset($_GET['id_forum']) && result(query("SELECT COUNT(*) FROM `forum_f` WHERE" . ((!isset($user) || $user['level'] == 0) ? " `adm` = '0' AND" : NULL) . " `id` = '" . intval($_GET['id_forum']) . "'"), 0) == 1

  && isset($_GET['id_razdel']) && result(query("SELECT COUNT(*) FROM `forum_r` WHERE `id` = '" . intval($_GET['id_razdel']) . "' AND `id_forum` = '" . intval($_GET['id_forum']) . "'"), 0) == 1

  && isset($_GET['id_them']) && result(query("SELECT COUNT(*) FROM `forum_t` WHERE `id` = '" . intval($_GET['id_them']) . "' AND `id_razdel` = '" . intval($_GET['id_razdel']) . "' AND `id_forum` = '" . intval($_GET['id_forum']) . "'"), 0) == 1

  && isset($_GET['id_post']) && result(query("SELECT COUNT(*) FROM `forum_p` WHERE `id` = '" . intval($_GET['id_post']) . "' AND `id_them` = '" . intval($_GET['id_them']) . "' AND `id_razdel` = '" . intval($_GET['id_razdel']) . "' AND `id_forum` = '" . intval($_GET['id_forum']) . "'"), 0) == 1

) {

  $forum = fetch_assoc(query("SELECT * FROM `forum_f` WHERE `id` = '" . intval($_GET['id_forum']) . "' LIMIT 1"));

  $razdel = fetch_assoc(query("SELECT * FROM `forum_r` WHERE `id` = '" . intval($_GET['id_razdel']) . "' AND `id_forum` = '" . intval($_GET['id_forum']) . "' LIMIT 1"));

  $them = fetch_assoc(query("SELECT * FROM `forum_t` WHERE `id` = '" . intval($_GET['id_them']) . "' AND `id_razdel` = '" . intval($_GET['id_razdel']) . "' AND `id_forum` = '" . intval($_GET['id_forum']) . "' LIMIT 1"));

  $post = fetch_assoc(query("SELECT * FROM `forum_p` WHERE `id` = '" . intval($_GET['id_post']) . "' AND `id_them` = '" . intval($_GET['id_them']) . "' AND `id_razdel` = '" . intval($_GET['id_razdel']) . "' AND `id_forum` = '" . intval($_GET['id_forum']) . "' LIMIT 1"));

  $post2 = fetch_assoc(query("SELECT * FROM `forum_p` WHERE `id_them` = '" . intval($_GET['id_them']) . "' AND `id_razdel` = '" . intval($_GET['id_razdel']) . "' AND `id_forum` = '" . intval($_GET['id_forum']) . "' ORDER BY `id` DESC LIMIT 1"));
...
// право на редактирование своего поста, если он поседний в теме
      ...
      if (!isset($err)) query("UPDATE `forum_p` SET `msg` = '" . my_esc($msg) . "' WHERE `id` = '$post[id]' LIMIT 1");
      ...
      query("DELETE FROM `forum_p` WHERE `id` = '" . intval($_GET['id_post']) . "' AND `id_them` = '" . intval($_GET['id_them']) . "' AND `id_razdel` = '" . intval($_GET['id_razdel']) . "' AND `id_forum` = '" . intval($_GET['id_forum']) . "' LIMIT 1");
...
if (isset($_GET['id_forum']) && result(query("SELECT COUNT(*) FROM `forum_f` WHERE" . ((!isset($user) || $user['level'] == 0) ? " `adm` = '0' AND" : NULL) . " `id` = '" . intval($_GET['id_forum']) . "'"), 0) == 1
  ...
  */
  ...
  query("UPDATE `notification` SET `read` = '1' WHERE `id_object` = '$them[id]' AND `type` = 'them_komm' AND `id_user` = '$user[id]'");

  /*------------очищаем счетчик этого обсуждения-------------*/

  if (isset($user)) {

    query("UPDATE `discussions` SET `count` = '0' WHERE `id_user` = '$user[id]' AND `type` = 'them' AND `id_sim` = '$them[id]' LIMIT 1");
  }
...
if (user_access('forum_for_create') && isset($_GET['act']) && isset($_GET['ok']) && $_GET['act'] == 'new' && isset($_POST['name']) && isset($_POST['opis']) && isset($_POST['pos'])) {
  ...
  if (!isset($err)) {
    ...
    query("INSERT INTO `forum_f` (`opis`, `name`, `pos`, `icon`) values('$opis', '$name', '$pos', '$icons')");
...
$q = query("SELECT * FROM `forum_f`" . ((!isset($user) || $user['level'] == 0) ? " WHERE `adm` = '0'" : NULL) . " ORDER BY `pos` ASC");
...
while ($forum = fetch_assoc($q)) {
  ...
  echo "<a href='/forum/$forum[id]/'><b>" . text($forum['name']) . "</b></a> <span style='color:#666;'>(" . result(query("SELECT COUNT(*) FROM `forum_p` WHERE `id_forum` = '$forum[id]'"), 0) . '/' . result(query("SELECT COUNT(*) FROM `forum_t` WHERE `id_forum` = '$forum[id]'"), 0) . ")\n";
...
if (user_access('forum_for_create') && (isset($_GET['act']) && $_GET['act'] == 'new' || result(query("SELECT COUNT(*) FROM `forum_f`"), 0) == 0)) {
  ...
  $pos = result(query("SELECT MAX(`pos`) FROM `forum_f`"), 0) + 1;
...
if (user_access('forum_for_create') && result(query("SELECT COUNT(*) FROM `forum_f`"), 0) > 0) {
