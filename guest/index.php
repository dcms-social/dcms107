...
/* Бан пользователя */ 
if (isset($user) && result(query("SELECT COUNT(*) FROM `ban` WHERE `razdel` = 'guest' AND `id_user` = '$user[id]' AND (`time` > '$time' OR `view` = '0')"), 0) != 0)
{
...
if (isset($user))
query("UPDATE `notification` SET `read` = '1' WHERE `type` = 'guest' AND `id_user` = '$user[id]'");
...
if (isset($_POST['msg']) && isset($user))
{
	...
	elseif (result(query("SELECT COUNT(*) FROM `guest` WHERE `id_user` = '$user[id]' AND `msg` = '".my_esc($msg)."' LIMIT 1"),0) != 0)
	{
	...
	elseif(!isset($err))
	{
		...
		*/
		...
		if (isset($ank_reply['id']))
		{
			...
			query("INSERT INTO `notification` (`avtor`, `id_user`, `type`, `time`) VALUES ('$user[id]', '$ank_reply[id]', 'guest', '$time')");
		}
    ...
    } elseif (result(query("SELECT COUNT(*) FROM `guest` WHERE `id_user` = '0' AND `msg` = '".my_esc($msg)."' LIMIT 1"),0) != 0) {
        $err = 'Ваше сообщение повторяет предыдущее';
    } elseif(!isset($err)) {
...
$k_post = result(query("SELECT COUNT(id) FROM `guest`"), 0);
...
$q = query("SELECT * FROM `guest` ORDER BY id DESC LIMIT $start, $set[p_str]");
...
echo '<div class="foot">';
echo '<img src="/style/icons/str.gif" alt="*"> <a href="who.php">В гостевой (' . result(query("SELECT COUNT(id) FROM `user` WHERE `date_last` > '".(time()-100)."' AND `url` like '/guest/%'"), 0) . ' чел.)</a><br />';
echo '</div>';
