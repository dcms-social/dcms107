...
if ($k_loads > 100) {
    if (result(query("SELECT COUNT(*) FROM `ban_ip` WHERE `min` <= '$iplong' AND `max` >= '$iplong'"), 0) == 0) query("INSERT INTO `ban_ip` (`min`, `max`, `prich`) values('$iplong', '$iplong', 'AntiDos')");
}
...
if (!isset($hard_process)) {
    $q = query("SELECT * FROM `cron` WHERE `id` = 'clear_tmp_dir'");
    if (rows($q) == 0) query("INSERT INTO `cron` (`id`, `time`) VALUES ('clear_tmp_dir', '$time')");
    $clear_dir = fetch_assoc($q);
    if ($clear_dir['time'] == NULL || $clear_dir['time'] < $time - 60 * 60 * 24) {
        $hard_process = true;
        query("UPDATE `cron` SET `time` = '$time' WHERE `id` = 'clear_tmp_dir'");
        if (function_exists('curl_init')) {
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, 'https://dcms-social.ru/curl.php?site=' . urlencode($_SERVER['HTTP_HOST']) . '&version=' . urlencode($set['dcms_version']) . '&title=' . urlencode($set['title']));
...
if (!isset($hard_process)) {
    ...
    if (rows($q) == 0) query("INSERT INTO `cron` (`id`, `time`) VALUES ('visit', '$time')");
    $visit = fetch_assoc($q);
    if ($visit['time'] == NULL || $visit['time'] < time() - 60 * 60 * 24) {
        ...
        if (result(query("SELECT COUNT(*) FROM `visit_everyday` WHERE `time` = '$last_day'"), 0) == 0) {
            $hard_process = true;
...
{ // Вырезает все нечитаемые символы
    if ($br != NULL) for ($i = 0; $i <= 31; $i++) $text = str_replace(chr($i), NULL, $text); else {
...
// бот
        ...
        if (!isset($users[$user_id])) {
            if (result(query("SELECT COUNT(*) FROM `user` WHERE `id` = '$user_id'"), 0) == 1) {
                $users[$user_id] = fetch_assoc(query("SELECT * FROM `user` WHERE `id` = '$user_id' LIMIT 1"));
                $tmp_us = fetch_assoc(query("SELECT `level`,`name` AS `group_name` FROM `user_group` WHERE `id` = '" . $users[$user_id]['group_access'] . "' LIMIT 1"));
...
function opsos($ips = NULL)
{
    ...
    if ($ips == NULL) $ips = $ip;
    $ipl = ip2long($ips);
    if (result(query("SELECT COUNT(*) FROM `opsos` WHERE `min` <= '$ipl' AND `max` >= '$ipl'"), 0) != 0) {
        $opsos = fetch_assoc(query("SELECT opsos FROM `opsos` WHERE `min` <= '$ipl' AND `max` >= '$ipl' LIMIT 1"));
...
if (!isset($hard_process)) {
    ...
    if (rows($q) == 0) query("INSERT INTO `cron` (`id`, `time`) VALUES ('everyday', '" . time() . "')");
    $everyday = fetch_assoc($q);
...
// удаляем все контакты, помеченные на удаление более месяца назад
        $qd = query("SELECT * FROM `users_konts` WHERE `type` = 'deleted' AND `time` < " . ($time - 60 * 60 * 24 * 30));
        while ($deleted = f_array($qd)) {
            query("DELETE FROM `users_konts` WHERE `id_user` = '$deleted[id_user]' AND `id_kont` = '$deleted[id_kont]'");

            if (result(query("SELECT COUNT(*) FROM `users_konts` WHERE `id_kont` = '$deleted[id_user]' AND `id_user` = '$deleted[id_kont]'"), 0) == 0) {
// если юзер не находится в контакте у другого, то удаляем и все сообщения
                query("DELETE FROM `mail` WHERE `id_user` = '$deleted[id_user]' AND `id_kont` = '$deleted[id_kont]' OR `id_kont` = '$deleted[id_user]' AND `id_user` = '$deleted[id_kont]'");
            }
        }
        $tab = query('SHOW TABLES FROM ' . $set['mysql_db_name']);
        for ($i = 0; $i < rows($tab); $i++) {
            query("OPTIMIZE TABLE `" . mysql_tablename($tab, $i) . "`"); // оптимизация таблиц
        }
    }
}
...
function admin_log($mod, $act, $opis)
{
    ...
    $q = query("SELECT * FROM `admin_log_mod` WHERE `name` = '" . my_esc($mod) . "' LIMIT 1");
    if (rows($q) == 0) {
        query("INSERT INTO `admin_log_mod` (`name`) VALUES ('" . my_esc($mod) . "')");
        $id_mod = mysql_insert_id();
    } else $id_mod = result($q, 0);

    $q2 = query("SELECT * FROM `admin_log_act` WHERE `name` = '" . my_esc($act) . "' AND `id_mod` = '$id_mod' LIMIT 1");
    if (rows($q2) == 0) {
        query("INSERT INTO `admin_log_act` (`name`, `id_mod`) VALUES ('" . my_esc($act) . "', '$id_mod')");
        $id_act = mysql_insert_id();
    } else $id_act = result($q2, 0);
