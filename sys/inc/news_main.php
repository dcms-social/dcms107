<?
$q = query("SELECT * FROM `news` WHERE `main_time` > '".time()."' ORDER BY `id` DESC LIMIT 1");

if (rows($q) == 1 && !$set['web'] && $user['news_read'] == 0)
{
	$news = fetch_assoc($q);
	echo '<div class="mess">';
 ...
 echo user::nick($news['id_user'],1,1,1).' '.vremja($news['time']).' ';
	echo ' <img src="/style/icons/komm.png" alt="*" /> (' . result(query("SELECT COUNT(*) FROM `news_komm` WHERE `id_news` = '$news[id]'"),0) . ')<br />';
