...
else
{
	...
	echo "<a href='/users.php'>Регистраций: ".result(query("SELECT COUNT(*) FROM `user`"), 0)."</a><br />\n";
	echo "<a href='/online.php'>Сейчас на сайте: ".result(query("SELECT COUNT(*) FROM `user` WHERE `date_last` > ".(time()-600).""), 0)."</a><br />\n";
	echo "<a href='/online_g.php'>Гостей на сайте: ".result(query("SELECT COUNT(*) FROM `guests` WHERE `date_last` > ".(time()-600)." AND `pereh` > '0'"), 0)."</a><br />\n";
