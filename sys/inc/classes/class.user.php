...
class user
{
     ...
     * / Ссылка и Ник юзера
     */
    public static function user_info($id, $field, $mass = FALSE)
    {
        static $user;

        if (empty($user[$field]))

            if (!$mass) $user[$field] = fetch_assoc(query('SELECT $field FROM `user` WHERE `id` = "' . $id . '"  '));
    ...
    public static function nick($user = 0, $url = 1, $on = 0, $medal = 0)
    {
        ...
        */
        ...
        $ank = fetch_assoc(query('SELECT `nick`, `date_last`, `rating`, `browser` FROM `user` WHERE `id` = "' . $user . '" LIMIT 1 '));
        ...
        if ($url == TRUE) $nick = ' <a href="/id' . $user . '">' . text($ank['nick']) . '</a> '; else
            $nick = text($ank['nick']);

        // Вывод значка онлайн
        if ($user != 0 && $ank['date_last'] > time() - 600 && $on == TRUE) {
            if ($ank['browser'] == 'wap') $online = ' <img src="/style/icons/online.gif" alt="WAP" /> '; else
    ...
    public static function avatar($user = 0, $type = 0)
    {
        ...
        */ global $time, $set;
        ...
        if ($user != 0) $ank = fetch_assoc(query('SELECT `pol`, `id`, `group_access` FROM `user` WHERE `id` = "' . $user . '" LIMIT 1 '));

        if ($user == 0) $ank = array('id' => '0', 'pol' => '1', 'group_access' => '0'); elseif (!$ank) $ank = array('id' => '0', 'pol' => '1', 'group_access' => '0');


        // Аватар

        if ($type == 0 || $type == 1) {
          $avatar = array(query("SELECT id,ras FROM `gallery_foto` WHERE `id_user` = '$user' AND `avatar` = '1' LIMIT 1"));
          if (isset($avatar['id'])) {
          if (is_file(H . 'sys/gallery/50/' . $avatar['id'] . '.' . $avatar['ras'])) $AVATAR = ' <img class="avatar" src="/foto/foto50/' . $avatar['id'] . '.' . $avatar['ras'] . '" alt="Avatar" /> '; else
        ...
        // Иконка пользователя
        if ($type == 0 || $type == 2) {
            if (result(query("SELECT COUNT(*) FROM `ban` WHERE `id_user` = '$user' AND (`time` > '$time' OR `navsegda` = '1')"), 0) != 0) {
                ...
                if ($ank['group_access'] > 7 && ($ank['group_access'] < 10 || $ank['group_access'] > 14)) {
                    if ($ank['pol'] == 1) $icon = '<img src="/style/user/1.png" alt="*" class="icon" id="icon_group" /> '; else
                    ...
                    if ($ank['pol'] == 1) $icon = '<img src="/style/user/3.png" alt="*" class="icon" id="icon_group" /> '; else
                        $icon = '<img src="/style/user/4.png" alt="*" class="icon" id="icon_group" /> ';
                } elseif (isset ($ank['status']) == 0) {
                    if ($ank['pol'] == 1) $icon = '<img src="/style/user/5.png" alt="" class="icon" id="icon_group" /> '; else
    ...
    static function get_user($ID = 0, $photo = 1)
    {
        ...
        */
        ...
        if ($user['id'] == $ID) {
            ...
            $ank = fetch_assoc(query('SELECT * FROM `user` WHERE `id` = "' . $ID . '" LIMIT 1'));
        }
        ...
        if ($ID == 0) {
            ...
            $tmp_us = fetch_assoc(query("SELECT `level`,`name` AS `group_name` FROM `user_group` WHERE `id` = '" . $ank['group_access'] . "' LIMIT 1"));
        ...
        if ($photo) {
            // Определяем аватар
            $avatar = array(query("SELECT id,ras FROM `gallery_foto` WHERE `id_user` = '$ID' AND `avatar` = '1' LIMIT 1"));

            if (is_file(H . 'sys/gallery/50/' . $avatar['id'] . '.' . $avatar['ras'])) $ank['avatar'] = ' <img class="avatar" src="/sys/gallery/50/' . $avatar['id'] . '.' . $avatar['ras'] . '" alt="Avatar" /> '; else
                $ank['avatar'] = ' <img class="avatar" src="/style/user/avatar.gif" width="50" alt="No Avatar" /> ';
        }

        // Вывод значка онлайн
        if ($ID != 0 && $ank['date_last'] > time() - 600) {
            if ($ank['browser'] == 'wap') $ank['online'] = ' <img src="/style/icons/online.gif" alt="WAP" /> '; else
        ...
        // Иконка пользователя
        if (result(query("SELECT COUNT(*) FROM `ban` WHERE `id_user` = '$ID' AND (`time` > '" . time() . "' OR `navsegda` = '1')"), 0) != 0) {
            $ank['icon'] = ' <img src="/style/user/ban.png" alt="*" class="icon" id="icon_group" /> ';
        } else {
            if ($ank['group_access'] > 7 && ($ank['group_access'] < 10 || $ank['group_access'] > 14)) {
                if ($ank['pol'] == 1) $ank['icon'] = '<img src="/style/user/2.png" alt="*" class="icon" id="icon_group" /> '; else
                    $ank['icon'] = '<img src="/style/user/120.png" alt="" class="icon"/> ';
            } elseif (($ank['group_access'] > 1 && $ank['group_access'] <= 7) || ($ank['group_access'] > 10 && $ank['group_access'] <= 14)) {
                if ($ank['pol'] == 1) $ank['icon'] = '<img src="/style/user/77.png" alt="*" class="icon" id="icon_group" /> '; else
                    $ank['icon'] = '<img src="/style/user/118.png" alt="*" class="icon" id="icon_group" /> ';
            } else {
                if ($ank['pol'] == 1) $ank['icon'] = '<img src="/style/user/23.png" alt="" class="icon" id="icon_group" /> '; else
