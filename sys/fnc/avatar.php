...
function avatar($ID, $link = false, $dir = '50', $w = '50')
{
	...
	* 
	*/
	$avatar = f_array(query("SELECT id,id_gallery,ras FROM `gallery_foto` WHERE `id_user` = '$ID' AND `avatar` = '1' LIMIT 1"));

	if (isset($avatar['id'])&&is_file(H."sys/gallery/$dir/$avatar[id].$avatar[ras]"))
	{
