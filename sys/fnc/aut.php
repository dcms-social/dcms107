...
function title()
{
	...
	if (isset($user))
	{
		...
		{			
			...
			$k_new = result(query("SELECT COUNT(`mail`.`id`) FROM `mail`
			...
			$k_new_fav = result(query("SELECT COUNT(`mail`.`id`) FROM `mail`
			 LEFT JOIN `users_konts` ON `mail`.`id_user` = `users_konts`.`id_kont` AND `users_konts`.`id_user` = '$user[id]'
			 ...
			 $k_frend = result(query("SELECT COUNT(id) FROM `frends_new` WHERE `to` = '$user[id]'"), 0); // Друзья
