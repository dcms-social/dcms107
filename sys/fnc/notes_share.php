...
$sql=query("SELECT * FROM `notes` WHERE `id`='".(int)$id."' LIMIT 1");
if(rows($sql)==1){
$post=fetch_assoc($sql);
if($post['share']==1){
...
$them=fetch_assoc(query("SELECT `id_forum`,`id`,`id_razdel` FROM `forum_t` WHERE `id`='".$post['share_id']."' LIMIT 1"));
?><div style="padding-left:5px;padding-top:5px;margin-left:17px;border-top:1px solid #b3b3b3;border-left:1px solid #b3b3b3;">
