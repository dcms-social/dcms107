...
{
	$q = query("SELECT * FROM `cron` WHERE `id` = 'backup_mysql'");

	if (rows($q) == 0)
		query("INSERT INTO `cron` (`id`, `time`) VALUES ('backup_mysql', '" . time() . "')");

	$backup = fetch_assoc($q);

	if (preg_match('#^[^@]*@[^@]*\.[^@]*$#iu',$set['mail_backup']) && ($backup['time'] == NULL || $backup['time'] < time()-60*60*24))
	{
		query("UPDATE `cron` SET `time` = '" . time() . "' WHERE `id` = 'backup_mysql'");
		$hard_process = true;
		...
		for($i = 0; $i < rows($tab); $i++)
		{
			...
			$res = @query("SHOW CREATE TABLE `$table`");
