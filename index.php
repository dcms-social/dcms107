    ...
    <div class="title">
        <center>
            <a href="/online.php" title="онлайн" style="color:#cdcecf; text-decoration: none">
                ...
                <font color="#ffffff"><?= result(query("SELECT COUNT(*) FROM `user` WHERE `date_last` > " . (time() - 600) . ""), 0) ?></font>
            </a>
            <font color="#fee300" size="2"> (</font>
            <font color="#ffffff">+<?= result(query("SELECT COUNT(*) FROM `guests` WHERE `date_last` > " . (time() - 600) . " AND `pereh` > '0'"), 0) ?></font>
