...
if(isset($_GET['id']))
{
	$id = fetch_assoc(query("SELECT * FROM `smile` WHERE `dir` = '" . intval($_GET['id']) . "' LIMIT 1"));

	if (result(query("SELECT COUNT(*) FROM `smile_dir` WHERE `id` = '" . intval($_GET['id']) . "'"),0) == 0)
	...
	if(isset($_GET['del']))
	{
		$del = fetch_assoc(query("SELECT * FROM `smile` WHERE `id` = '" . intval($_GET['del']) . "' LIMIT 1"));

		@unlink(H.'style/smiles/' . $del['id'] . '.gif');
		query("DELETE FROM `smile` WHERE `id` = '".intval($_GET['del'])."'");
	...
	if(isset($_GET['act']) && $_GET['act'] == 'add_smile' && isset($_GET['ok']) && isset($_POST['forms']))
	{
		...
		for ($i = 0; $i < $forms; $i++)
		{
			...
			isset($_POST["smile_$i"]))
			{
				...
				query("INSERT INTO `smile` (`smile`,`dir`) values('$smile','" . intval($_GET['id']) . "')");
				$ID = mysql_insert_id();
...
{
	$q = query("SELECT * FROM `smile` WHERE `dir` = '" . intval($_GET['delete']) . "'");
	
	while($post = f_array($q))
	{
		@unlink(H.'style/smiles/' . $post['id'] . '.gif');
		query("DELETE FROM `smile` WHERE `id` = '" . $post['id'] . "'");
	}
	
	query("DELETE FROM `smile_dir` WHERE `id` = '" . intval($_GET['delete']) . "'");
...
if (isset($_GET['id']))
{
	...
	========================
	*/
	$k_post = result(query("SELECT COUNT(*) FROM `smile` WHERE `dir`='".intval($_GET['id'])."'"),0);
	...
	$q = query("SELECT * FROM `smile` WHERE `dir`='" . intval($_GET['id']) . "' ORDER BY id DESC LIMIT $start, $set[p_str]");
		...
		*/
		if(isset($_GET['edit']) && $_GET['edit'] == $post['id'])
		{
			...
			if (isset($_POST['sav']))
			{
				...
				if (!isset($err))
				{
					query("UPDATE `smile` SET `smile` = '$smile' WHERE `id` = '$post[id]'");
...
if(isset($_GET['act']) && $_GET['act'] == 'add_kat')
{
	if(isset($_POST['save']))
	{
		...
		if(!isset($err))
		{
			query("INSERT INTO `smile_dir` (`name` ) VALUES ('$name')");
...
========================
*/
$k_post = result(query("SELECT COUNT(*) FROM `smile_dir`"),0);
...
while($post = f_array($q))
{
	...
	<a href="?id=<?=$post['id']?>"><?=text($post['name'])?></a> (<?=result(query("SELECT COUNT(*) FROM `smile` WHERE `dir` = '$post[id]'"),0)?>)
	...
	*/
	if (isset($_GET['edit']) && $_GET['edit'] == $post['id'])
	{
		if (isset($_POST['sav']))
		{
			...
			if (!isset($err))
			{
				query("UPDATE `smile_dir` SET `name` = '" . $name . "' WHERE `id` = '" . intval($_GET['edit']) . "'");
