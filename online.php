...
$k_lider = result(query("SELECT COUNT(*) FROM `liders` WHERE `time` > '$time'"),0);

$liders = fetch_assoc(query("SELECT * FROM `liders` WHERE `time` > '$time' ORDER BY rand() LIMIT 1"));
...
$q = query("SELECT id, ank_city, pol, ank_d_r, ank_m_r, ank_g_r, ank_o_sebe, url, level, ip, ip_xff, ip_cl, ua, date_last FROM `user` WHERE `date_last` > '".(time()-600)."' ORDER BY `date_last` DESC LIMIT $start, $set[p_str]");
