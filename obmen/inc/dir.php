...
if (isset($_GET['trans']))
$trans = fetch_assoc(query("SELECT * FROM `obmennik_files` WHERE `id` = '".intval($_GET['trans'])."' AND `id_user` = '$user[id]' LIMIT 1"));
...
if (!isset($_GET['act']) && !isset($_GET['trans']))
{
	...
	{
		$dir_user = fetch_assoc(query("SELECT * FROM `user_files`  WHERE `id_user` = '$user[id]' AND `osn` = '1'"));
...
$q=query("SELECT * FROM `obmennik_dir` WHERE `dir_osn` = '/$l' OR `dir_osn` = '$l/' OR `dir_osn` = '$l' ".(user_access('obmen_dir_edit')?"":"AND `my` = '0'")." ORDER BY `name`,`num` ASC");
...
$q=query("SELECT * FROM `obmennik_files` WHERE `id_dir` = '$id_dir' ORDER BY `$sort_files` DESC");
...
$k_f=0;
$k_n=0;
$q3=query("SELECT * FROM `obmennik_dir` WHERE `dir_osn` like '$post[dir]%'");
while ($post2 = fetch_assoc($q3))
{
$k_f=$k_f+result(query("SELECT COUNT(*) FROM `obmennik_files` WHERE `id_dir` = '$post2[id]'"),0);
$k_n=$k_n+result(query("SELECT COUNT(*) FROM `obmennik_files` WHERE `id_dir` = '$post2[id]' AND `time_go` > '" . $ftime . "'",$db), 0);
}
$k_f=$k_f+result(query("SELECT COUNT(*) FROM `obmennik_files` WHERE `id_dir` = '$post[id]'"),0);
$k_n=$k_n+result(query("SELECT COUNT(*) FROM `obmennik_files` WHERE `id_dir` = '$post[id]' AND `time_go` > '" . $ftime . "'",$db), 0);
