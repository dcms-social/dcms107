...
$dir_my=fetch_assoc(query("SELECT * FROM `obmennik_dir` WHERE `id` = '$trans[id_dir]' LIMIT 1"));
//if (result(query("SELECT COUNT(*) FROM `obmennik_files` WHERE `size` = '$trans[size]'"),0)>1 && $dir_my['my']!=1)$err = 'Такой в файл уже есть в обменнике';
...
if (!$ras_ok)$err='Неверное расширение файла';
	...
	if (!$err)
	{
	query("UPDATE `obmennik_files` SET `id_dir` = '$dir_id[id]' WHERE `id` = '$trans[id]' LIMIT 1");
