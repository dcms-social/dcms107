...
$new=null; }
$k_post=result(query("SELECT COUNT(*) FROM `notes` WHERE `private`='0'"),0);
$k_page=k_page($k_post,$set['p_str']);
$page=page($k_page);
$start=$set['p_str']*$page-$set['p_str'];

$q=query("SELECT * FROM `notes` $order LIMIT $start, $set[p_str]");
...
echo " <img src='/style/icons/action_share_color.gif'> (".result(query("SELECT COUNT(`id`)FROM `notes` WHERE `share_id`='".$post['id']."' AND `share_type`='notes'"),0).") </font>";
echo "  </div>\n";
