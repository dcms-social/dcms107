...
if (!isset($err)){
query("INSERT INTO `notes_dir` (`msg`, `name`) values('$msg', '$title')");
query("OPTIMIZE TABLE `notes_dir`");
...
$kount=result(query("SELECT COUNT(*) FROM `notes_dir` WHERE `id` = '$id_dir' "),0);
}
...
$k_post=result(query("SELECT COUNT(*) FROM `notes`  WHERE `id_dir` = '$id_dir'"),0);
$k_page=k_page($k_post,$set['p_str']);
$page=page($k_page);
$start=$set['p_str']*$page-$set['p_str'];
$q=query("SELECT * FROM `notes` WHERE `id_dir` = '$id_dir' $order LIMIT $start, $set[p_str]");
...
$k_n= result(query("SELECT COUNT(*) FROM `notes` WHERE `id` = $post[id] AND `time` > '".$ftime."'",$db), 0);
...
==================================
*/
$k_post=result(query("SELECT COUNT(*) FROM `notes_dir` "),0);
$q=query("SELECT * FROM `notes_dir` ORDER BY `id` DESC");
...
$k_pp=result(query("SELECT COUNT(*) FROM `notes`  WHERE `id_dir` = '$post[id]'"),0);
$k_nn=result(query("SELECT COUNT(*) FROM `notes`  WHERE `id_dir` = '$post[id]' AND `time` > '$ftime'"),0);
if ($k_nn>0)
...
//$k_n= result(query("SELECT COUNT(*) FROM `notes` WHERE `id_dir` = $post[id] AND `time` > '".$ftime."'",$db), 0);
