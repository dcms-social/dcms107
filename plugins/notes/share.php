...
/* Ban user */
if (isset($user) && result(query("SELECT COUNT(`id`) FROM `ban` WHERE `razdel` = 'notes' AND `id_user` = '$user[id]' AND (`time` > '$time' OR `view` = '0' OR `navsegda` = '1')"), 0)!=0)
{
...
title();
aut(); only_reg();
$not=query("SELECT * FROM `notes` WHERE `id`='".intval($_GET['id'])."' LIMIT 1");
if(rows($not)==0){
echo "<div class='err'>Такой записи не существует</div>";
}elseif(result(query("SELECT COUNT(`share_id_user`)FROM `notes` WHERE `id_user`='".$user['id']."' AND `share_id`='".intval($_GET['id'])."' AND `share_type`='notes' LIMIT 1"),0)==1){
...
if(isset($_POST['ok'])){
query("INSERT INTO `notes`(`id_user`,`name`,`msg`,`share`,`share_text`,`share_id`,`share_id_user`,`share_name`,`time`) values('".$user['id']."','".text($notes['name'])."','".my_esc($_POST['share_text'])."','1','".my_esc($notes['msg'])."','".$notes['id']."','".$notes['id_user']."','".my_esc($notes['name'])."','".$time."')");
$id=mysql_insert_id();
msg('Ок всё крч');
