...
$q = query("SELECT * FROM `smile_dir` ORDER BY id ASC");

while ($dir = fetch_assoc($q))
{
	// Лесенка
	...
	<a href="/plugins/smiles/dir.php?id=<?=$dir['id']?>"><?=text($dir['name'])?></a> (<?=result(query("SELECT COUNT(*) FROM `smile` WHERE `dir` = '$dir[id]'"),0)?>)
	</div>
	<?
}
