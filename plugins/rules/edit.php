...
{
	if (isset($_GET['id']) && result(query("SELECT COUNT(*) FROM `rules_p` WHERE `id` = '".intval($_GET['id'])."'"),0)==1)
	{	$post=fetch_assoc(query("SELECT * FROM `rules_p` WHERE `id` = '".intval($_GET['id'])."' LIMIT 1"));
		...
		if (isset($_POST['change']) && isset($_GET['id']) && isset($_POST['name']) && $_POST['name']!=NULL)
		{
			...
			query("UPDATE `rules_p` SET `msg` = '$msg' WHERE `id` = '$id' LIMIT 1");
...
{
	if (isset($_GET['id']) && result(query("SELECT COUNT(*) FROM `rules` WHERE `id` = '".intval($_GET['id'])."'"),0)==1)
	{
		$post = fetch_assoc(query("SELECT * FROM `rules` WHERE `id` = '".intval($_GET['id'])."' LIMIT 1"));
		$ank = fetch_assoc(query("SELECT * FROM `user` WHERE `id` = $post[id_user] LIMIT 1"));
		...
		if (isset($_POST['change']) && isset($_GET['id']))
		{
			...
			query("UPDATE `rules` SET `msg` = '$name' WHERE `id` = '$id' LIMIT 1");
			query("UPDATE `rules` SET `title` = '$title' WHERE `id` = '$id' LIMIT 1");
			query("UPDATE `rules` SET `url` = '$url' WHERE `id` = '$id' LIMIT 1");
			query("UPDATE `rules` SET `name_url` = '$name_url' WHERE `id` = '$id' LIMIT 1");
			$_SESSION['message'] = 'Пункт меню успешно изменен';
