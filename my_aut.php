...
$k_post = result(query("SELECT COUNT(*) FROM `user_log` WHERE `id_user` = '$user[id]'"),0);
...
$q = query("SELECT * FROM `user_log` WHERE `id_user` = '".$user['id']."' ORDER BY `id` DESC  LIMIT $start, $set[p_str]");

while ($post = fetch_assoc($q))
{
	
	$ank = f_array(query("SELECT * FROM `user` WHERE `id` = '$user[id]' LIMIT 1"));
